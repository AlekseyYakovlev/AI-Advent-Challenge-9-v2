...
@pytest.mark.asyncio
async def test_get_settings_falls_back_to_global(authenticated_client: AsyncClient) -> None:
    """Per-chat GET should inherit global settings when no override exists."""
    chat_resp = await authenticated_client.post("/api/v1/chats", json={"title": "Fallback chat"})
    ...
    resp = await authenticated_client.get(f"/api/v1/settings?chat_id={chat_id}")
...
@pytest.mark.asyncio
async def test_per_chat_settings_override_global(authenticated_client: AsyncClient) -> None:
    """Per-chat settings should override global values for that chat only."""
    chat_resp = await authenticated_client.post("/api/v1/chats", json={"title": "Override chat"})
    chat_id = chat_resp.json()["id"]
    ...
    per_chat = await authenticated_client.get(f"/api/v1/settings?chat_id={chat_id}")
    global_settings = await authenticated_client.get("/api/v1/settings")
...
@pytest.mark.asyncio
async def test_update_global_settings(authenticated_client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_context_length_persists(authenticated_client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_no_compression_strategy(authenticated_client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_update_per_chat_creates_row(authenticated_client: AsyncClient) -> None:
    """Per-chat PUT should create a dedicated settings row."""
    chat_resp = await authenticated_client.post("/api/v1/chats", json={"title": "Settings row"})
    chat_id = chat_resp.json()["id"]
